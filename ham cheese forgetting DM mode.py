...
class MyAgent(ACTR):
    ...
    partial = Partial(DM, strength=1.0, limit=-1.0) # turn on partial matching                                                 # usually this is strength divided by number of slots
    partial.similarity('customer1', 'customer1', 0.3) # set the similarity between customer1 and customer3 - not so similar

    def init():
        DM.add('customer:customer1 condiment:mustard')
        DM.add('customer:customer2 condiment:hot_peppers')
        DM.add('customer:customer_anoying_person condiment:spit')
    ...
    def ham(focus='sandwich ham'):
        ...
        focus.set('customer2 condiment')         
                                        
    def condiment(focus='customer2 condiment'):  # customer will spread activation to 'customer mustard'
